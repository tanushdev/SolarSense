...
│   ├── temporal_features.py     ← Dynamic Lag & Neupert features
...
│   ├── 03_physics_feature_analysis.ipynb
...
```
      ← Historical flare similarity search
```
...
The following models MUST be benchmarked independently.
...
Deploy ONLY the best-performing model.

Do NOT deploy an ensemble unless it provides statistically significant improvement in:
...
Only required if historical memory retrieval is enabled.
